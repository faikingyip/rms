...
class NewTag(QWidget):
    on_tag_created = Signal()
    ...
    async def on_save_clicked_handler(self):
        await app_ops_tag.create_tag(
            schema_tag.SchemaTagCreate(name=self.line_edit_tag_name.text().strip())
        )
        self.on_tag_created.emit()
